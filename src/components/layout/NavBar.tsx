...
export default function NavBar() {
  ...
  return (
    <nav
      class="flex items-center justify-between px-5 h-[52px] bg-[#2c2c2c] border-b-4 border-[#5dade2]"
      aria-label="Main navigation"
    >
      <A
        href="/budget-visualizer/transactions"
        class="text-lg font-semibold text-[#ecf0f1] no-underline hover:text-[#5dade2] transition-colors"
      >
        Budget Visualizer
      </A>
      ...
      <button
        type="button"
        class="flex items-center gap-2 cursor-pointer px-2.5 py-1.5 rounded-md border-none bg-transparent font-[inherit] transition-colors hover:bg-[#5dade2]/15"
        onClick={handleLoginLogout}
        aria-label={authState.isUserAuthenticated ? 'User menu' : 'Log in'}
      >
        <div
          class={`size-8 rounded-full flex items-center justify-center transition-colors ${authState.isUserAuthenticated ? 'bg-[#5dade2]' : 'bg-[#555]'}`}
          role="img"
          aria-hidden="true"
        >
          <Show
            ...
            fallback={
              <svg class="size-5 text-[#ecf0f1]" viewBox="0 0 24 24" fill="currentColor">
        ...
        <span class="text-[#ecf0f1] text-sm" data-testid="navbar-login-label">
